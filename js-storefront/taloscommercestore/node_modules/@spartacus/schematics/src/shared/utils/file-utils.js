...
function injectService(config) {
    ...
    config.propertyType = (_a = config.propertyType) !== null && _a !== void 0 ? _a : core_1.strings.classify(config.serviceName);
...
function insertCommentAboveIdentifier(sourcePath, source, identifierName, comment, identifierType = typescript_1.default.SyntaxKind.Identifier) {
    const classNode = ast_utils_1.getSourceNodes(source).find((node) => node.kind === typescript_1.default.SyntaxKind.ClassDeclaration);
    if (!classNode) {
        return [new change_1.NoopChange()];
    }
